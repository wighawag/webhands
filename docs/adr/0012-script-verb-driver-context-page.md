...
- **Source from `--file <path>` OR an inline string OR stdin** (exactly one). The common case is `webhands script --file ./flow.js` (the agent writes a flow file and points the verb at it, like the baseline does); an inline `webhands script "<js>"` gives `eval`-style parity for a short snippet; stdin is the optional third source. (AMENDED 2026-06-30: narrowed to a single FILE-PATH source: `npx webhands script ./flow.js`. See the amendment at the end.)
...
## Amendment (2026-06-30): one source, a FILE PATH only

The "Source from `--file <path>` OR an inline string OR stdin (exactly one)" design above is NARROWED to a SINGLE source: a required FILE-PATH positional. `npx webhands script ./flow.js` is now the only form; the verb reads that path and runs its contents.

- The inline-string positional, the `--file` flag, and the stdin pipe are all REMOVED. A bare `webhands script` (no path) fails loud (the positional is required); a missing/unreadable path fails loud with a typed, non-cryptic error (the `invalid-script` error shape is preserved, and the message names the path).
- WHY: one source, one rule. The file-first workflow is exactly what a raw-Playwright agent already writes (a flow file, then run it), so making it the ONLY workflow keeps the `script`-vs-Playwright comparison honest and removes the "three ways to do one thing" surface. The `--file` flag was redundant once the positional itself became the path.
- UNCHANGED: everything else in this ADR stands. The DRIVER context (the full live Playwright `page`), the built-in `scriptHand`, the two boundaries (the in-process `page` API is not an ADR-0003 surface; the RETURN stays ADR-0003 seam-clean and serializable), and the trust boundary (the same page-script `eval` surface, loopback-only, NOT hand/module loading). Only HOW the source is SUPPLIED changed (a file path), not what `script` does or returns.
- The `script` was the ONLY stdin consumer, so the `readScriptStdin`/`readProcessStdin`/`resolveScriptSource` multi-source machinery was removed cleanly (no dead code), replaced by a single `readFile(path, 'utf8')` at the call site.
