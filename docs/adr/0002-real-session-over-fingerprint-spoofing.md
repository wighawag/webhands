...
Refined by [ADR-0014](0014-attach-to-the-users-own-chrome-is-the-anti-bot-answer.md): measured against a commercial bot manager, the practical answer is ATTACHING to a browser the user started themselves (`serve --real-chrome`), not a hardened Playwright launch. Stealth is one tell removed, not the recipe. The principle below is unchanged; ADR-0014 is it taken further.
