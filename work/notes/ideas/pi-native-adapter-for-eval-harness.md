...
The deferred pi adapter was framed in the spec `agent-capability-eval-harness` (## Resolved decisions D1, ## Out of Scope) as merely "pi-native session/liveness niceties". The live demo on 2026-06-29 showed that UNDERSOLD it. `pi --mode json` emits one JSON event per line AS THINGS HAPPEN (`session.subscribe(event => writeRawStdout(JSON.stringify(event)))` in pi `packages/coding-agent/src/modes/print-mode.ts`), and those events carry real structure the shell adapter throws away:
