---
title: A review verb that crystallizes a just-driven session into a tested hand scaffold
...
The idea: a **`review`** verb that CRYSTALLIZES the just-driven session into a
reusable hand. After the agent finally gets the flow working, one `review` call
emits a starting-point hand module (plus a human-readable notes/review markdown of
...
## The input source: what does `review` crystallize FROM?

`review` needs to know what the successful flow WAS. Three sources, layered, in
...
top, so `review` works everywhere and gets better where the environment allows.
...
- **Backbone (portable, ground-truth) - webhands' OWN verb trace.** The `serve`
  ...
  per-session VERB TRACE, `review` crystallizes from webhands' own record of what
...
- **Enrichment 2 (opt-in, explicit path) - a `--session-file <path>` arg.** When
  ...
  `review` can mine the real reasoning + step sequence. This is a PLAIN PATH the
  ...
  input `review` accepts when given.
...
After `review` emits a scaffold (e.g. `./hands/checkout.js`), it can IMMEDIATELY
...
- **Adopting it is the human's act.** LOADING the file as a real hand means naming
  ...
  dependency). `review` MUST NOT auto-load what it generated - that is exactly the
  arbitrary-in-process-RCE hazard the `agent-provided-hand-via-cli-arg` idea
  guards. `review` EMITS and TESTS; the human REVIEWS the file and ADOPTS it.

The safe pipeline end to end:

> explore -> `review` (emit scaffold + notes, validate via `script`) -> human
> reviews the file -> human names it in `hands.json` -> next run is one cheap call.

## Design sketch (to be pinned in a PRD, not here)

- `review` inputs: the session VERB TRACE (from `serve`, the backbone), optional
  `--summary <text>`, optional `--session-file <path>`. Omitting the enrichments
  still produces a scaffold from the trace.
- `review` outputs: (1) a hand MODULE scaffold at a caller-named path (a `Hand`
...
- Validation: `review --test` (or a follow-up) runs the scaffold via `script`
  against the live page and reports pass/fail, so the emitted hand is tested.
- HARD RULE: `review` never adds to `hands.json` and never loads the module. Emit
...
- **webhands discovering the transcript location itself - OUT (separate note).**
  ...
  harness. `review` accepts a `--session-file` PATH it is handed; producing that
  path is the `harness-seam-session-awareness` idea's job, optional and graceful.
- **A perfect, ready-to-ship hand - NOT the goal.** `review` emits a SCAFFOLD (a
...
1. What exactly does the `serve` verb trace record, and how is it REDACTED? A
   trace of a login flow contains typed credentials; the crystallized scaffold and
   the notes markdown must not leak secrets. (Likely a PRD-level decision with an
   ADR for the redaction contract.)
2. Is `review` one verb with flags (`--summary`, `--session-file`, `--test`,
   `--out`) or a small family? Lean: one verb, flagged, mirroring `script`'s
   single-source simplicity.
3. Does `review` target the WHOLE session or a caller-named SLICE (e.g. "just the
...
4. How much of the hand does `review` actually write vs leave as TODO? A faithful
...
5. Relationship to `--hand`/`allowAgentHands` (the `agent-provided-hand-via-cli-arg`
   idea): `review` is the AUTHORING half (emit+test a scaffold), that idea is the
...
`review` verb that crystallizes the just-driven session into a tested hand
