...
- **SETTLE FIRST (the crux of this task): how does the Playwright-only agent get a driving surface WITHOUT going through webhands?** This is the load-bearing design fork and it should be the FIRST thing resolved (consider a short spike before committing to the seam reuse). The fork: does the agent drive its OWN Playwright (its process must have Playwright + a browser available and write its own automation), or does the harness hand it a page/endpoint? Note the contract weight: a shell-launched agent driving 'its own Playwright' is a MUCH heavier agent contract than the webhands case (where the agent just runs `npx webhands <verb>`). If reusing the generic shell `AgentUnderTest` seam does not survive contact with this (e.g. the Playwright-only agent needs a fundamentally different launch shape), do NOT force it: surface a needs-attention and re-scope rather than build on a premise that broke. Either way the harness must NOT route this agent through the webhands verb surface (that defeats the baseline). Decide and RECORD which option you took.
- **The harness still needs a live serve session for its OWN reads, even in Playwright-only mode.** The harness's end-state assertion stays the harness's OWN and MAY use webhands read verbs for the verdict (the verdict is the harness's mechanism, not the agent's toolkit), so 'Playwright-only' constrains the AGENT, NOT the harness: a webhands serve session is still alive for the harness to read the end state, even though the agent never touches webhands. Do NOT conclude 'Playwright-only ⇒ no serve session at all'.
