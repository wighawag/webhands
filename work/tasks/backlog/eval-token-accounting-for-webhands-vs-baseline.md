...
- **Adapter-specific capture (best-effort, honest).** The capture lives in the adapter that knows the agent:
  ...
  - This is additional motivation for the deferred pi-native adapter (see `work/notes/ideas/pi-native-adapter-*` if present): a pi-native adapter parses pi's `--mode json` event stream as a first-class capability and gets exact, structured usage for free.
