...
- **hand** — a capability MODULE that closes over the Page and contributes verbs. Given a scoped-but-live hand-context `{pwPage, context, ensureOpen}` (the live Playwright `Page`, the `BrowserContext`, and the per-session lifecycle guard), a hand returns named verbs plus an optional `dispose`. webhands' own eight verbs are themselves built-in hands composed over an internal **hand-host** primitive (proven by self-application). `hand` is a THIRD axis orthogonal to `transport` and `verb`: a **transport** is HOW we reach the browser, a **verb** is one agent-facing action, a **hand** is a capability module closing over the Page. Guards: a hand is **NOT a verb** (one hand may contribute several verbs plus in-process logic); a hand is **NOT a transport** (it does not `open` sessions — it gets the live Playwright page directly). Hands are offered only by a transport that can hand over live page access (the Playwright transport); a transport that cannot do page-level operations offers no hands. In Phase 1 hands are trusted, local, in-process peers with ZERO isolation, and `Hand`/`HandContext` stay package-internal (public only in Phase 2); see `docs/adr/0006` (refines `0003`/`0004`).
