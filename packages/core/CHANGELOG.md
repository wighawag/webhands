...
- 5e1c9c4: **`serve --socket <path>` serves the session over a UNIX SOCKET, so an account that cannot reach loopback can still drive a browser.**
...
- cd6c499: Run the `packages/core` test files sequentially (`fileParallelism: false`).
